...
def SG_model(datetime_date):
	...
	date_part = datetime_date # .replace(tzinfo=TZ)
	...
	day_of_year = (date_part - datetime.datetime(date_part.year, 1, 1, tzinfo=TZ)).days + 1
	...
	try:
		f1 = pow(np.cos(theta), 1.3644)
		f2 = pow(np.e, (-0.0013 * (90 - zenith_angle)))
